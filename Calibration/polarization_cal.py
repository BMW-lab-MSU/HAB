...
def polarization_cal(exposureTime,gain,output_dir):
    ...
    Degree = 0
    pic_numb = 0
    rot_stage.Set_Degree(Degree+rotation_stage_offset)
    time.sleep(2)
    ...
    while True:
        ...
        if pic_numb >=5:
            ...
            print()
            if Degree > 360:
                rot_stage.Set_Degree(0+rotation_stage_offset)
                break
            time.sleep(1)
            rot_stage.Set_Degree(Degree+rotation_stage_offset)
            time.sleep(2)
            ...
            print("Degree of rotation stage:", rot_stage.cur_degree)
        ...
        for i in range(len(cams)):
            ...
            filename = color+"-"+"PolCal"+"-"+filename
            ...
            if type(imgs[i]) != type(None):
                ...
                print("not saved")
    if not crash:  
        print("Pol Cal done:", "Calibration of "+CamNames[0]+" complete.")  
        for cam in cams:
            cam.stop()
            cam.close()
        rot_stage.close()
        ...
        print("Crash:", "The program crashed at",Degree) 
